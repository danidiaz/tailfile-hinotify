...
-}
tailFile :: FilePath
         -> (forall t r. (MonadTrans t, MonadIO (t IO)) => Producer Data.ByteString.ByteString (t IO) r -> t IO (void, r)) -- ^ Scary type, but any reasonably polymorphic (say, over 'MonadIO') function that consumes a 'Producer' can go here.
         -> IO void
